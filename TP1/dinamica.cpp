...
bool algunoCubre(vector<aspersor>& aspersores,double largo, double inicio){
    bool existe=false;
    for(int i=0; i<aspersores.size();i++){
        if(aspersores[i].empieza <=inicio){
            existe= true;
        }
    }
    return existe;
...
int menor(vector<aspersor>& aspersores, int inicio, int fin,int i,double largo, vector<int>& costos){
    if(i==aspersores.size() || aspersores[i-1].termina>=largo){
        return 0;
    }else if(!algunoCubre(aspersores,aspersores[i].termina,inicio)){
        return inf;
    }
    costos[i]=10000;
...
int menorCosto(vector<aspersor>& aspersores,double largo,double inicio,double final,int i, vector<int>& costos){
    ...
    if(!algunoCubre(aspersores,aspersores[i].termina,inicio)){// O(n)
        return -1;
    }else{
        int j=i;
        if(costos[j]==-2){
